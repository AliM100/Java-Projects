...
public class SandwichController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle bundle) {
		...
		int row=0;
		if(sz==0) {
			alerts.Noti("No Available Sandwiches");
		}
		else {
			for(int i=0;i<sz;i++) {
				Label ln,lp;
				...
				try {
					...
					ln.setText(sl.get(i).getName());
					lp=(Label) n.getChildren().get(4);
					lp.setText(""+sl.get(i).getPrice()+" L.L");
					v=(ImageView) n.getChildren().get(2);
					v.setFitHeight(185);
					v.setFitWidth(260);
					v.setImage(sl.get(i).getimage().getImage());
	...
	public void add(String sname,double sprice) {
		try {
		    ...
		    s.setOnHidden(e->{
		    	try {
		    		if(addToCartController.getFlag()==1) {
					    ...
					    if(q!=0) {
					    	if(f==1) {
					    		DB.updateC(loginController.getUcart(), DB.getPid(sname),q);
					    		f=0;
					    	}
					    	else{
					    		DB.addPC(loginController.getUcart(),DB.getPid(sname),q);
					    	}
